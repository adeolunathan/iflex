...
const ModelResultsPanel: React.FC<ModelResultsPanelProps> = ({
  ...
  const processTimeSeriesData = (rawData: any[]) => {
    ...
    rawData.forEach(item => {
      ...
      if (!periodMap[period]) {
        periodMap[period] = { timestamp: period };
      }
  ...
  const generateMetrics = (timeSeriesArray: any[]) => {
    ...
    const previousPeriod = timeSeriesArray.length > 1 ? timeSeriesArray[timeSeriesArray.length - 2] : null;
    
    // Get all components excluding timestamp
    const componentNames = Object.keys(lastPeriod).filter(key => key !== 'timestamp');
  ...
  return (
    <Paper sx={{ width: '100%' }}>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={activeTab} onChange={handleTabChange} aria-label="model results tabs">
          <Tab label="Dashboard" />
          <Tab label="Chart" />
          <Tab label="Table" />
        </Tabs>
